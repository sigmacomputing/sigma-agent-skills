...
The empirical coverage matrix (how each kind was verified live, POST + readback + render) is the source of truth: **`reference/specification/trellis-chart-support.md`**.
...
was confirmed against a real org via `POST /v2/workbooks` with `dryRun: true`, and
