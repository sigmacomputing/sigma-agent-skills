...
**`contents.agents[]` array**, a sibling of `pages` / `layout` / `settings`.
The agent is surfaced by a flat `contents.elements[]` `chat` entry that
...
> asset — see `SKILL.md` → *Fetching the compiled asset*). `contents.agents[]`,
...
Note the `contents` wrapper — `agents` is a sibling of `pages` / `layout` /
`settings`, **inside** `contents` (see `schema.md`):
...
(`contents.agents[0].greeting: Invalid value: string`):
...
> `layout`, `settings`, `agents`; `name`/`folderId`/`description` sit outside `contents`). A
