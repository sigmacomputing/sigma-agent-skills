...
A workbook carries a theme under `contents.settings`, alongside `pages` and `layout`:
...
> `settings.theme.overrides` — inside `contents`, alongside `pages`/`layout`. The individual
...
The **full overrides schema is in the public OpenAPI** (the `contents` POST/PUT request body and GET response). The OpenAPI is the source of truth for every field; the list below is the verified summary.
...
- Categorical (`categorical: true`): `{ "settings": { "theme": { "overrides": { "categoricalScheme": theme[:categorical] } } } }`
  — **deep-merge** at the workbook level (inside `contents`, a sibling of `pages`/`layout`), not per-element.
