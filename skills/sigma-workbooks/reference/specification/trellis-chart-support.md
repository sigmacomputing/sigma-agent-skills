...
`POST /v2/workbooks` (Data page: a source table with a few-member category
...
`GET /v2/workbooks/{id}?includeContents=true` was read back and the `trellis` key compared to
