...
membership from the order of `contents.elements`.
...
Each `<Page id>` matches a `contents.pages[].id`, `contents.overlays[].id`, or
`contents.panels[].id`. Each `elementId` matches a flat
`contents.elements[].id`; placing it beneath a block is what assigns it there.
...
A `kind: "container"` entry in `contents.elements[]` is a grouping placeholder
...
`contents.elements[]`, placed inside the container in layout. They reference the
...
> representation. However, both a `POST /v2/workbooks` dry run (`dryRun: true`) and
> a real `POST /v2/workbooks` currently reject a replay with
...
> `scripts/internal/probe-release-contract.rb` keeps a plain child in its main
...
> `contents.elements[]`; local dangling-reference checks must not reject it.
...
**JSON element** (`contents.elements[]`) — `tabs[]` entries are labels only:
...
in `contents.elements[]`; `<Tab>` order ties them to the labels above.
...
- **Building it:** hand-authoring the position-mapped `<Tab>` block is error-prone. Use `Composition.tabbed_container(id:, tabs:, grid_column:, grid_row:, tab_bar_alignment: 'left')` in `scripts/lib/composition.rb` — `tabs:` is `[{name:, inner:}]`, where `inner` is the tab's bare-`<Element>` XML (built with `Composition.band`/`Composition.le` or by hand). It returns `{element:, layout:}`, ready to add to `contents.elements[]` and `contents.layout`.
...
Each layout `elementId` must match a flat `contents.elements[].id` exactly.
...
> `GET /v2/workbooks/{id}?includeContents=true` with both `contents.panels[]` entries, the
> `contents.settings.navigation` block, and `<Panel>` layout nodes intact, and
> the panels render. The shapes below match `components.schemas.Workbook`
> → `contents.panels` / `contents.settings.navigation`.
>
> **Workspace entitlement gate (still applies).** `contents.settings.navigation`
...
`contents.panels[]` holds page **headers** and page **sidebars** — chrome
...
(`GET /v2/workbooks/{id}?includeContents=true`, 2026-08-10):
...
Every `<Panel id>` must match a `contents.panels[].id`, and every element it
places must exist in the flat `contents.elements` array (same
...
### `contents.settings.navigation` — the on/off switch

A panel renders **only** when its corresponding setting is `enabled`.
Defining a panel in `contents.panels[]` without enabling it here is inert,
...
`settings.navigation` + `contents.panels` for built-in header/sidebar chrome;
use a `navigation` element when the menu needs to live inside the page grid
alongside other content.

To study real grid-container idioms, fetch an existing multi-page workbook's spec (`GET /v2/workbooks/{id}?includeContents=true`, see SKILL.md Steps 1–2). The OpenAPI doesn't model the `layout` XML string, so a live spec is the way to see production layout.
