...
3. **Published data entry is a UI-only element setting.** Use *kebab → Set
   ...
   draft.” The setting is absent from a `GET ...?includeContents=true` readback, so Code Rep cannot set or
...
versions linked tables were also dropped from the readback entirely; if inherited
...
To inspect an input table's **structure regardless of the readback** (handy on older
orgs where linked tables are hidden from a `?includeContents=true` readback, or for a quick column dump),
use the element endpoints (linked columns label as `Col (ParentName)`):

```bash
# list every element (incl. input tables hidden from the readback on older orgs)
