...
They live in flat `contents.elements[]`; layout places them.
...
> plus an `Id` suffix. Verified by A/B creates plus `GET .../v2/workbooks/{id}?includeContents=true` readback
...
| Modal/drawer overlay (`contents.overlays`) | `on-primary-cta-click`, `on-secondary-cta-click`, `on-close` |
...
`contents.elements`; page scope resolves to the host page and can clear
...
Modal and drawer metadata lives in `contents.overlays`, not
`contents.pages`. Drawer overlays slide from an edge and modals center. Their
content elements remain in flat `contents.elements` and layout assigns them.
...
Per the OpenAPI, `overlayId` must reference an entry in `contents.overlays`;
pointing it at an ordinary page or element is invalid.

> **Layout gotcha:** an overlay needs its own `<Page>`
> block in `contents.layout`, with the overlay id. When you build
...
Each shape passed a `POST /v2/workbooks` dry run (`dryRun: true`), was created for real via
`POST /v2/workbooks`, and came back byte-intact from `GET /v2/workbooks/{id}?includeContents=true` on a
...
> `contents.pages[0]: Invalid type: "page"` — because a button isn't a valid
