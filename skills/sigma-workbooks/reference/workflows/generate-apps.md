...
(`POST /v2/workbooks`). Architecture details live in the matching recipe,
not here.
...
`agent.include` false and omit `contents.agents` and `chat`. If **yes**, do
...
5. **Workbook agent.** Fixtures stay agent-free; compose after clone.
   If `agent.include` is false, omit `contents.agents` and any `chat`
   element. If **true**:
...
version. The setting is absent from a `GET ...?includeContents=true` readback.
...
Fixtures stay **agent-free** (`contents.agents` and `chat` are composed in
