...
The live OpenAPI requires flat `contents.elements` and metadata-only pages.
`PUT /v2/workbooks/{id}/contents` accepts only `{contents: {...},
documentVersion?}` — it's contents-only, so `name`/`folderId` sent alongside
are silently ignored. Data-model specs are a separate surface and may retain
nested `pages[].elements`.
...
- `contents` has `schemaVersion`, `kind: workbook`, `elements`, and `pages`.
- `pages`, `overlays`, and `panels` contain metadata only.
- Every literal element appears once in `contents.elements`.
- Every element is placed in `contents.layout`.
...
spec and compare it with the submitted contents:
...
| Error | Meaning |
|---|---|
| `contents.pages[].elements is no longer supported` | Move literal elements to `contents.elements`; use layout for placement. |
| `element '<id>' is not placed in layout` | Add a layout placement for that flat element. |
| `Invalid kind: contents.elements[N]` | The selected element/control subtype has the wrong inner shape. |
