...
3. **Data elements and kinds.** For each visible element, name the Sigma `kind`
   ...
   not a read-only `table`. A conversation rail is `contents.agents[]` plus a
...
- assistant rail → `contents.agents[]` + `chat`;
...
Inline SVG data URIs may pass a `POST /v2/workbooks` dry run (`dryRun: true`) but still trigger a
