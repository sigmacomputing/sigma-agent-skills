...
#
# Commands:
#   pull <workbook-id> [dir]     GET contents and explode (refuses to overwrite a dirty rep)
#   status [dir]                 element-level diff: working files vs last-synced snapshot
#   push [dir]                   reassemble -> drift-check -> validate -> PUT contents (or POST create)
...
#   verify <spec-file>           dry-run: POST /v2/workbooks with dryRun: true — zero-persistence
...
# contents object. Legacy flat artifacts (and legacy `document`-keyed ones)
# are accepted on import and normalized here.
def canonical_spec(spec)
  Sigma::CodeRep.metadata(spec).merge('contents' => Sigma::CodeRep.document(spec))
end
...
def explode(spec, dir, raw_yaml:, manifest_extra: {})
  ...
  top = strip_response_only(spec).merge('contents' => doc)
...
def diff_specs(old_spec, new_spec)
  ...
  lines = []
  ot = strip_response_only(old_spec).reject { |k, _| k == 'contents' }
  nt = strip_response_only(new_spec).reject { |k, _| k == 'contents' }
...
#
# Sigma's dry-run validate (POST /v2/workbooks with dryRun: true) catches the
# element-predicate case but NOT control wiring (measured 2026-08-20): a control filtering a nonexistent column
...
def cmd_pull(args, force:)
  ...
  raw = api(:get, "/v2/workbooks/#{wb_id}?includeContents=true")
...
def cmd_verify(args)
  ...
  body = create_body(load_yaml_file(path)).merge('dryRun' => true)
  result = YAML.load(api(:post, '/v2/workbooks', YAML.dump(body)))
  # dryRun's response omits `valid` entirely on the clean-and-no-warnings path
  # (bare metadata, same as a real create would return) -- only `valid: false`
  # (invalid) or `valid: true` (valid but with non-fatal warnings) sets it.
  if result['valid'] == false
    puts 'valid: false'
    (result['errors'] || []).each { |e| puts "  - #{e['summary']}" }
    (result['warnings'] || []).each { |w| puts "  (warning) #{w['summary']}" }
    exit 1
  else
    puts 'valid: true'
    (result['warnings'] || []).each { |w| puts "  (warning) #{w['summary']}" }
...
def cmd_push(args, force:, validate: true)
  ...
  if wb_id && !force
    remote = canonical_spec(YAML.load(api(:get, "/v2/workbooks/#{wb_id}?includeContents=true")))
  ...
  if wb_id
    api(:put, "/v2/workbooks/#{wb_id}/contents", YAML.dump(put_body(clean_body)))
  else
    die 'create mode: workbook.yaml must include folderId' unless clean_body['folderId']
    res = canonical_spec(YAML.load(api(:post, '/v2/workbooks', YAML.dump(create_body(clean_body)))))
    wb_id = res['workbookId'] or die "create response had no workbookId:\n#{res.inspect}"
    mf['workbookId'] = wb_id
  end

  readback = canonical_spec(YAML.load(api(:get, "/v2/workbooks/#{wb_id}?includeContents=true")))
  FileUtils.mkdir_p(File.join(dir, '.sigma'))
...
def cmd_summarize(args)
  ...
  spec = if File.directory?(target)
           ...
           canonical_spec(YAML.load(api(:get, "/v2/workbooks/#{target}?includeContents=true")))
         end
  doc = document(spec)
  ...
  unless File.exist?(OPENAPI_CACHE)
    ...
    unless res
      ...
      die "failed to fetch the compiled OpenAPI from #{OPENAPI_URL}#{detail}. If you are " \
          ...
          '(GET /v2/workbooks/{id}?includeContents=true) or a per-endpoint reference page ' \
